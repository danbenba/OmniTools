@echo off
title OmniTools - Disable Defender
echo.
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
echo.
echo.
echo NOTE: It is highly recommended to install Platform Updates sometimes (monthly) because they affect Windows protection as well, like System Guard!
echo.
echo Disable Tamper and Real Protection in Defender - RESTART!
echo start windowsdefender:
echo Disable Real Protection in Defender - Run "Microsoft Defender Disable.bat" - RESTART!
echo Run "Microsoft Defender Disable.bat" again - RESTART!
echo.
echo Alternatively run in Safe Mode
echo Run - msconfig - Boot - Safe Boot - Minimal - Restart
echo Run "MD Disable.bat" TWICE - RESTART!
echo Run - msconfig - General - Normal Startup
echo.
